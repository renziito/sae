...
(function ($) {
  ...
  Social.prototype.init = function () {
    ...
    if (typeof stepsForm != 'undefined' && 1 == 2) {
      this.$status.length && new stepsForm(this.$status.get(0), {
        onSubmit: function (form) {
          _this.$status.find('.status-form-inner').addClass('hide');
          _this.$status.find('.final-message').html('<i class="fa fa-check-circle-o"></i> Comentario Enviado').addClass('show');
  ...
  function Plugin(option) {
    return this.each(function () {
      ...
      if (!data)
        $this.data('pg.social', (data = new Social(this, options)));
