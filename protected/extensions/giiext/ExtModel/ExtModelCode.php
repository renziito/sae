...
class ExtModelCode extends CCodeModel {
    ...
    public function generateRules($table) {
        ...
        if ($required !== array())
            $rules[] = "array('" . implode(', ', $required) . "', 'required', 'message' => '{attribute} no debe estar vacio.')";
        if ($integers !== array())
            $rules[] = "array('" . implode(', ', $integers) . "', 'numerical', 'integerOnly'=>true,'message' => '{attribute} solo debe ser numeros.')";
        if ($numerical !== array())
